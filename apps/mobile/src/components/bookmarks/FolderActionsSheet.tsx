...
export function FolderActionsSheet({ visible, onDismiss, folder, anchor, onDeleted }: FolderActionsSheetProps) {
  ...
  const doSetDeviceLock = async () => {
    ...
    try {
      ...
      const message = errorMessage(cause, "Could not enable the device lock.");
      setError(message);
      toast.error(message);
    } finally {
      setRemoving(false);
  ...
  const menuVisible = visible && !!folder && (mode === "menu" || mode === "lockChoice");
  const panelVisible = visible && !!folder && mode !== "menu" && mode !== "lockChoice";

  return (
    <>
    <ContextMenu visible={menuVisible} onDismiss={onDismiss} anchor={anchor}>
      {folder && mode === "menu" ? (
        <>
      ...
      {folder && mode === "lockChoice" ? (
        <>
          {lockTypesSupported ? (
            <>
              <ContextMenuItem icon="finger-print-outline" label="Device lock" onPress={doSetDeviceLock} />
          ...
          <ContextMenuItem icon="chevron-back" label="Back" onPress={() => showMode("menu")} />
        </>
      ) : null}
    </ContextMenu>
    <FloatingPanel visible={panelVisible} onDismiss={onDismiss}>
      {folder && mode === "rename" ? (
        <>
